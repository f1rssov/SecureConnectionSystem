...
func startServ() {
    ...
    // Слушаем порт 2222 на всех интерфейсах (0.0.0.0)
    listener, err := net.Listen("tcp", "0.0.0.0:2222") // <--- ИЗМЕНЕНИЕ ЗДЕСЬ
    if err != nil {
        log.Fatal("Ошибка при старте сервера:", err)
    }
    log.Println("SSH сервер запущен на 0.0.0.0:2222 и требует аутентификации клиента")
