...
// sshServerAddr будет установлено во время сборки с помощью ldflags
// Например: go build -ldflags="-X 'main.sshServerAddr=0.tcp.ngrok.io:12345'"
var sshServerAddr string

func init() {
	// Устанавливаем значение по умолчанию, если оно не было переопределено ldflags
	if sshServerAddr == "" {
		sshServerAddr = "127.0.0.1:2222" // Значение по умолчанию для локального тестирования
	}
}
...
func startClient() {
	...
	log.Println("Подключение к SSH серверу...")
	conn, err := ssh.Dial("tcp", sshServerAddr, config) // <--- ИЗМЕНЕНИЕ ЗДЕСЬ
	if err != nil {
